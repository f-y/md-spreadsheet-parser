...
def convert_table(obj: Any) -> WitTable:
    ...
    res.rows = obj.rows
    res.alignments = (
        [convert_alignment_type(x) for x in obj.alignments]
...
def convert_conversion_schema(obj: Any) -> WitConversionSchema:
    ...
    res.boolean_pairs = (
        str(obj.boolean_pairs) if obj.boolean_pairs is not None else None
    )
    res.custom_converters = (
        json.dumps(obj.custom_converters or {})
        if obj.custom_converters is not None
        else None
    )
    res.field_converters = (
        json.dumps(obj.field_converters or {})
